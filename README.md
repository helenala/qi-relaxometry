...
Magnons, the quanta of spin waves, represent collective spin precession modes. In thin magnetic films, thermally excited magnons generate stray fields at the film surface, which can be detected using quantum impurities like nitrogen-vacancy (NV) centers. These lattice defects in diamond can couple with magnon stray fields. Assuming thermal occupancy of magnon modes, we investigate the magnetization dynamics of magnons propagating through thin magnetic insulators, employing the Landau-Lifshitz-Gilbert equation.

Our work involves developing a numerical model to predict and analyze the NV center's response to nearby magnons in thin films. The simulation incorporates a static bias field with an arbitrary orientation relative to the NV center's quantization axis, leveraging the tetrahedral symmetry of diamond. This extended model addresses challenges posed by current measurement techniques, which struggle to precisely align the bias field with the NV center. The module builds on and extends the quantum impurity resonance (QIR) theory outlined in Rustagi et al. (2020) [1].
